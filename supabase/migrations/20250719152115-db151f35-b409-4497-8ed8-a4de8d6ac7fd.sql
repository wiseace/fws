-- Fix the send_admin_reply function to resolve SELECT DISTINCT ORDER BY issue
...
BEGIN
  ...
  -- Remove DISTINCT and use subquery to fix ORDER BY issue
  SELECT admin_id INTO target_admin_id
  FROM (
    SELECT DISTINCT admin_id, created_at
