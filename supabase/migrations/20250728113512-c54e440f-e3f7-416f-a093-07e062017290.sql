...
ADD COLUMN user_type user_type DEFAULT 'provider';
...
-- Add unique constraint to prevent duplicate plans for same user type and currency
ALTER TABLE public.subscription_pricing 
DROP CONSTRAINT IF EXISTS unique_plan_currency;

ALTER TABLE public.subscription_pricing 
ADD CONSTRAINT unique_plan_currency_usertype UNIQUE (plan, currency_code, user_type);

-- Insert default seeker pricing (example pricing - admin can modify)
