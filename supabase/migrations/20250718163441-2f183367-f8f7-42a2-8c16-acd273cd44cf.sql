-- Update smart search function to only return verified providers with active subscriptions
CREATE OR REPLACE FUNCTION public.smart_search_providers(search_term text DEFAULT ''::text, search_location text DEFAULT ''::text, min_price numeric DEFAULT NULL::numeric, max_price numeric DEFAULT NULL::numeric, availability_only boolean DEFAULT false, user_lat numeric DEFAULT NULL::numeric, user_lng numeric DEFAULT NULL::numeric)
 RETURNS TABLE(user_id uuid, name text, email text, phone text, profile_image_url text, skills text[], tags text[], service_location text, city_or_state text, availability_status text, price_range_min numeric, price_range_max numeric, last_active timestamp with time zone, service_id uuid, service_name text, service_description text, service_category text, service_price_min numeric, service_price_max numeric, match_score integer)
 LANGUAGE plpgsql
AS $function$
BEGIN
  RETURN QUERY
